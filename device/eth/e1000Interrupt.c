...
//uint32 rdfpc = e1000_io_readl(ethptr->iobase, 0x02430);	// RDFPC
//kprintf("[e1000Interrupt]: rdfpc %x\r\n", rdfpc);
//kprintf("[e1000Interrupt]: status %x\r\n", status);
	if (status == 0) {
...
//		kprintf("[e1000Interrupt]: ICR_RXT0\r\n");
		ethptr->rxIrq++;
		e1000_rxPackets(ethptr);
	}

	if (status & E1000_ICR_TXDW) {
//		kprintf("[e1000Interrupt]: ICR_TXDW\r\n");
		ethptr->txIrq++;
