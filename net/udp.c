...
/*kprintf( (udptr->udstate != UDP_FREE) ? "TRUE": "FALSE");
kprintf("\r\n");
kprintf( (currpkt->net_udpdport == udptr->udlocport)? "TRUE":"FALSE" );
kprintf("\r\n");
kprintf( ((udptr->udremport == 0) || (currpkt->net_udpsport == udptr->udremport))? "TRUE":"FALSE" );
kprintf("\r\n");
kprintf(((udptr->udremip==0) || (currpkt->net_ipsrc == udptr->udremip)) ? "TRUE": "FALSE");
kprintf("\r\n");
*/
	for (i=0; i<UDP_SLOTS; i++) {
	    udptr = &udptab[i];
