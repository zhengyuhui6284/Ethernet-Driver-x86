...
uint32	getlocalip(void)
{
	...
	/* allocate a UDP entry with IP addr, server port, client port */			
	...
	/* until now DHCP Discover message has been prepared */
	...
	for (i = 0; i < DHCP_RETRY; i++) {
		...
		for (j=0; j<3; j++) {
			...
			/* DHCP OPTION:
			 * Discover: 53-DHCP discover, 50-192.168.1.100 requested
			 * Offer: 53-DHCP offer, 1-255.255.255.0 subnet, 
			 * 		3-192.168.1.1 router, 54-192.168.1.1 server
			 * Request: 53-DHCP request, 50-192.168.1.100 requested, 
			 * 		54-192.168.1.1 server
			 * ACK: 53-DHCP ACK, 1-255.255.255.0 subnet mask, 3-192.168.1.1 router, 
			 * 		51-1day IP lease, 54-192.168.1.1 server */
			...
			if (msgtype == 0x02) {	/* offer - send request	*/
				...
				udp_send(IP_BCAST, UDP_DHCP_SPORT, IP_THIS, UDP_DHCP_CPORT, (char *)&dmsg2,
